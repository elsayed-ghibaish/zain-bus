...
export default function AllBookingUi() {
  ...
  const handleCompletedChange = (
    ...
    setselectedCompleted(event.target.value);
  };
  ...
  const filteredData = Bookings.data.filter((item: any) => {
    ...
    if (TimingEnd.length > 0 && !TimingEnd.includes(item.attributes.end_time)) {
      return false;
    }
    ...
    if (selectedcompleted) {
      if (selectedcompleted === "false") {
        return item.attributes.completed === false;
      } else {
        return (
          item.attributes.completed === selectedcompleted ||
          item.attributes.completed === true
        );
      }
    }
    ...
    switch (selectedstatus) {
      case "pending":
        return item.attributes.trip_status === "pending";
      case "confirmed":
        return item.attributes.trip_status === "confirmed";
      case "cancelled":
        return item.attributes.trip_status === "cancelled";
      case "rejected":
        return item.attributes.trip_status === "rejected";
    }
        ...
        <div className="bg-slate-300 border-2 border-white shadow-sm rounded-t-lg grid grid-cols-1 gap-x-7 gap-y-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 p-5 print:hidden print:bg-none">
          ...
          <div className="sm:col-span-1">
            ...
            >
              ...
              <option value="pending">فى انتظار التاكيد</option>
            </select>
          </div>
